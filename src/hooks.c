...
/*   Updated: 2018/12/13 20:10:06 by dromanic         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (((k == NUM_1 || k == ONE) && ft_swith(&f->n1))
	|| ((k == NUM_2 || k == TWO) && ft_swith(&f->n2))
	|| ((k == NUM_3 || k == THREE) && ft_swith(&f->n3))
	|| ((k == NUM_4 || k == FOUR) && ft_swith(&f->n4))
	|| ((k == NUM_5 || k == FIVE) && ft_swith(&f->n5))
	|| ((k == NUM_6 || k == SIX) && ft_swith(&f->n6))
	|| ((k == NUM_7 || k == SEVEN) && ft_swith(&f->n7))
	|| ((k == NUM_8 || k == EIGHT) && ft_swith(&f->n8))
	|| (k == Q && ft_swith(&f->q)) || (k == W && ft_swith(&f->w))
	|| (k == E && ft_swith(&f->e)) || (k == T && ft_swith(&f->range)
		&& (p->col_step = (f->range ? 0xFFFFFF : 0xFFFFFFFF) / p->depth))
	|| (k == Y && ft_swith(&f->carioid)) || (k == G && ft_swith(&f->alt_color)))
	{
		redraw_fract_or_img(env, env->param, 0);
		return (1);
	}
	...
	|| (k == H && ft_swith(&f->hints)) || (k == N && ft_swith(&f->values))
	|| (k == M && ft_swith(&f->menu)))
	{
		redraw_fract_or_img(env, env->param, 1);
		return (1);
